...
class Style{
    ...
    void SetXY(int arg0,int arg1);  // (gm) —Å—Ç–∞—Ä—ã–π: SetTwoFields -> SetXY
    // 0x00462FB0 undefined4 __thiscall
    int IsMapTileSpecial(int param_1,unsigned int param_2);  // (gm) —Å—Ç–∞—Ä—ã–π: IsSlotAttrFour -> IsMapTileSpecial
    // 0x00462FD0
    int StyleGetSlotAttr(void);
    // 0x00463020 byte __thiscall
    unsigned char IsPointInRect(unsigned char param_1,unsigned char param_2);  // (gm) ÒÚ‡˚È: IsInsideBounds -> IsPointInRect
    ...
    // 0x004BF570
    int GetStylePalette(void);  // (gm) —Å—Ç–∞—Ä—ã–π: GetRemapColour -> GetStylePalette
    // 0x004BF5D0
