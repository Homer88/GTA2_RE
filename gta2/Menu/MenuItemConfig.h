...
class  MenuItemConfig{
public:
    char IsActive;           ///< Флаг активности
    char SubType;            ///< Подтип элемента
    short Param1;            ///< Параметр 1 (ID/индекс)
    short Param2;            ///< Параметр 2 (счетчик/состояние)
    short SelectedIndex;     ///< Выбранный индекс
   ...
   /// static_assert(sizeof(MenuItemConfig) == 108, "MenuItemConfig must be 108 bytes");
    ...
    void Initialize(char isActive, char subType,
