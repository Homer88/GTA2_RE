#include "CrashData.h"
CrashData* gCrashData;

    // 0x0044C870


int CrashData::IsCarInvolved(void){
        return 0;
    }


    // 0x004769E0


void CrashData::SetTarget3(int param_2,int param_3){

    }


    // 0x00476A00


void CrashData::SetSecondary(int param_2){

    }


    // 0x00476A10


void CrashData::SetField24(int param_1,int param_2){

    }


    // 0x00476A20


void CrashData::SetTextPos(int param_1,int param_2){

    }


    // 0x00476A30


void CrashData::DrawText(void){

    }


    // 0x00476A60


void CrashData::UpdateBoth(void){

    }


    // 0x00476A90


int CrashData::GetValue(void){
        return 0;
    }


    // 0x00476AA0


void CrashData::SetActiveFlag(void){

    }


    // 0x00476AB0


void CrashData::SetInactive(void){

    }


    // 0x00476AD0


int CrashData::SetActive(void){
        return 0;
    }


    // 0x004895E0


void CrashData::ResetBuffer8(int arg0){

    }


    // 0x004895F0


void CrashData::ResetBufferState(void){

    }


    // 0x00489680


int CrashData::ProcessCrash(void){
        return 0;
    }


    // 0x00489B10


void CrashData::ParkVehicleUpdate(int param_1){

    }


    // 0x00489BC0


int CrashData::AddCrash(void){
        return 0;
    }


    // 0x00493540


int CrashData::IsCarEqual(void){
        return 0;
    }