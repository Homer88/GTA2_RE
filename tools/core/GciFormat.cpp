...
bool GciFile::ParseText(const std::string& text)
{
	...
	// Собираем комментарий-заголовок в начале файла.
	// Заголовок — это строки ДО первого блока машины. Блок машины всегда
	// начинается со строки вида "{Name}" (без '*'). Заголовок же содержит
	// строки с '*' (разделители "{****...}") и служебные строки
	// ("Generated By : ..." и т.п.) без '*'. Поэтому считаем строку частью
	// заголовка, пока она не является блоком "{Name}".
	std::string header;
	while (i < lines.size()) {
		std::string t = Trim(lines[i]);
		if (t.empty()) { if (!header.empty()) header += "\n"; i++; continue; }
		// Строка блока машины: "{Имя}", без '*' -> заголовок закончился.
		bool isCarName = (t.size() >= 3 && t[0] == '{' && t[t.size()-1] == '}' &&
